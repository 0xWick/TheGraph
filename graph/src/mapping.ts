...
export function handleGameEnded(event: GameEnded): void {
  ...
  // needs to unique across all entities of the same type
  ...
  // Entity fields can be set based on event parametrs
...
export function handlePlayerJoined(event: PlayerJoined): void {
  // Entities can be loaded from the store using a string ID; this ID
  // needs to be unique accross all entities of the same type
  let entity = Game.load(event.params.gameId.toString());
  // Entities only exist after they have been saved to the store;
  // 'null' checks alow to create entities on demand
  if (!entity) {
    return;
  }
